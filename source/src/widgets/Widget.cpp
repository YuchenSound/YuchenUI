...
void Widget::removeChild(UIComponent* child) {
    ...
    if (m_ownerContent && child->canReceiveFocus()) {
...
void Widget::clearChildren() {
    if (m_ownerContent) {
        for (auto& child : m_children) {
            if (child && child->canReceiveFocus()) {
...
bool Widget::dispatchMouseEvent(const Vec2& position, bool pressed, const Vec2& offset, bool isMove) {
    ...
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it) {
        ...
        bool handled = false;
        if (isMove) {
            handled = (*it)->handleMouseMove(position, absPos);
        } else {
            handled = (*it)->handleMouseClick(position, pressed, absPos);
        }
...
void Widget::setOwnerContent(IWindowContent* content) {
    ...
    for (auto& child : m_children) {
        if (child) {
            ...
            if (content && child->canReceiveFocus()) {
