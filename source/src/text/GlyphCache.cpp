...
GlyphCache::GlyphCache(GraphicsContext* context, float dpiScale)
    : m_context(context)
...
void GlyphCache::destroy()
{
    ...
    for (auto& atlas : m_atlases) {
        if (atlas->textureHandle) {
            m_context->destroyTexture(atlas->textureHandle);
...
void GlyphCache::createNewAtlas()
{
    ...
    atlas->textureHandle = m_context->createTexture2D(
        atlasWidth,
        atlasHeight,
