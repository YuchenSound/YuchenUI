...
TextRenderer::TextRenderer(GraphicsContext* context)
    : m_context(context)
    ...
    , m_shapedTextCache()
{
    YUCHEN_ASSERT_MSG(context != nullptr, "GraphicsContext cannot be null");
}
...
bool TextRenderer::initialize(float dpiScale)
{
    ...
    YUCHEN_ASSERT_MSG(m_context != nullptr, "GraphicsContext is null");
    YUCHEN_ASSERT_MSG(m_context->isInitialized(), "GraphicsContext not initialized");
    
    m_dpiScale = dpiScale;
    
    YUCHEN_ASSERT_MSG(FontManager::getInstance().isInitialized(), "FontManager not initialized");
    
    m_glyphCache = std::make_unique<GlyphCache>(m_context, m_dpiScale);
