...
//==========================================================================================
// IGraphicsBackend Interface - Initialization

bool D3D11Renderer::initialize(void* platformSurface, int width, int height, float dpiScale)
{
    YUCHEN_ASSERT_MSG(!m_isInitialized, "Already initialized");
    YUCHEN_ASSERT_MSG(width >= Config::Window::MIN_SIZE && width <= Config::Window::MAX_SIZE,
                     "Invalid width");
    YUCHEN_ASSERT_MSG(height >= Config::Window::MIN_SIZE && height <= Config::Window::MAX_SIZE,
                     "Invalid height");
    YUCHEN_ASSERT_MSG(dpiScale > 0.0f && dpiScale <= 3.0f, "Invalid DPI scale");
    
    m_hwnd = static_cast<HWND>(platformSurface);
    m_width = width;
    m_height = height;
    m_dpiScale = dpiScale;
...
//==========================================================================================
// IGraphicsBackend Interface - Frame Management
...
//==========================================================================================
// IGraphicsBackend Interface - Texture Management
...
void* D3D11Renderer::createTexture2D(uint32_t width, uint32_t height, TextureFormat format)
{
    ...
    DXGI_FORMAT dxgiFormat = (format == TextureFormat::R8_Unorm) ?
...
void D3D11Renderer::updateTexture2D(void* texture, uint32_t x, uint32_t y,
...
bool D3D11Renderer::loadShaders()
{
    ...
    if (FAILED(m_device->CreateVertexShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_rectVS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/BasicPS.cso", &blob)) return false;
    if (FAILED(m_device->CreatePixelShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_rectPS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/TextVS.cso", &blob)) return false;
    if (FAILED(m_device->CreateVertexShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_textVS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/TextPS.cso", &blob)) return false;
    if (FAILED(m_device->CreatePixelShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_textPS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/ImageVS.cso", &blob)) return false;
    if (FAILED(m_device->CreateVertexShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_imageVS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/ImagePS.cso", &blob)) return false;
    if (FAILED(m_device->CreatePixelShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_imagePS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/ShapeVS.cso", &blob)) return false;
    if (FAILED(m_device->CreateVertexShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_shapeVS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/ShapePS.cso", &blob)) return false;
    if (FAILED(m_device->CreatePixelShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_shapePS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/CircleVS.cso", &blob)) return false;
    if (FAILED(m_device->CreateVertexShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_circleVS))) { blob->Release(); return false; }
    blob->Release();
    
    if (!loadShaderFromFile(L"shaders/CirclePS.cso", &blob)) return false;
    if (FAILED(m_device->CreatePixelShader(blob->GetBufferPointer(),
        blob->GetBufferSize(), nullptr, &m_circlePS))) { blob->Release(); return false; }
    blob->Release();
...
bool D3D11Renderer::createInputLayouts()
{
    ...
    HRESULT hr = m_device->CreateInputLayout(rectLayout, 6, vsBlob->GetBufferPointer(),
        vsBlob->GetBufferSize(), &m_rectInputLayout);
    vsBlob->Release();
    ...
    hr = m_device->CreateInputLayout(textLayout, 3, vsBlob->GetBufferPointer(),
        vsBlob->GetBufferSize(), &m_textInputLayout);
    vsBlob->Release();
    if (FAILED(hr)) return false;
    ...
    hr = m_device->CreateInputLayout(imageLayout, 2, vsBlob->GetBufferPointer(),
        vsBlob->GetBufferSize(), &m_imageInputLayout);
    vsBlob->Release();
    if (FAILED(hr)) return false;
    ...
    hr = m_device->CreateInputLayout(shapeLayout, 2, vsBlob->GetBufferPointer(),
        vsBlob->GetBufferSize(), &m_shapeInputLayout);
    vsBlob->Release();
    if (FAILED(hr)) return false;
    ...
    hr = m_device->CreateInputLayout(circleLayout, 5, vsBlob->GetBufferPointer(),
        vsBlob->GetBufferSize(), &m_circleInputLayout);
    vsBlob->Release();
    if (FAILED(hr)) return false;
    ...
    for (size_t i = 0; i < commands.size(); ++i) {
        ...
        switch (cmd.type) {
            ...
            case RenderCommandType::DrawText: {
                ...
                m_textRenderer->shapeText(cmd.text.c_str(), cmd.westernFont,
                    cmd.chineseFont, cmd.fontSize, shapedText);
                if (!shapedText.isEmpty()) {
                    std::vector<TextVertex> vertices;
                    m_textRenderer->generateTextVertices(shapedText, cmd.textPosition,
                        cmd.textColor, cmd.westernFont, cmd.fontSize, vertices);
    ...
    for (size_t i = 0; i < commands.size(); ++i) {
        ...
        switch (cmd.type) {
            ...
            case RenderCommandType::FillTriangle:
                renderTriangle(cmd.triangleP1, cmd.triangleP2, cmd.triangleP3,
                    cmd.color, 0.0f, true);
                break;
                
            case RenderCommandType::DrawTriangle:
                renderTriangle(cmd.triangleP1, cmd.triangleP2, cmd.triangleP3,
                    cmd.color, cmd.borderWidth, false);
                break;
            ...
            case RenderCommandType::DrawCircle:
                renderCircle(cmd.circleCenter, cmd.circleRadius, cmd.color,
                    cmd.borderWidth, false);
                break;
...
void D3D11Renderer::renderRectangle(const Rect& rect, const Vec4& color,
...
void D3D11Renderer::renderRectBatch(const std::vector<RenderCommand>& commands,
...
void D3D11Renderer::generateImageVertices(const Rect& destRect, const Rect& sourceRect,
...
void D3D11Renderer::computeNineSliceRects(const Rect& destRect, const Rect& sourceRect,
...
void D3D11Renderer::generateNineSliceVertices(void* texture, const Rect& destRect,
                                             ...
                                             std::vector<float>& outVertices)
{
    ...
    srcSlices[1] = Rect(sourceRect.x + margins.left, sourceRect.y,
        sourceRect.width - margins.left - margins.right, margins.top);
    srcSlices[2] = Rect(sourceRect.x + sourceRect.width - margins.right, sourceRect.y,
        margins.right, margins.top);
    srcSlices[3] = Rect(sourceRect.x, sourceRect.y + margins.top, margins.left,
        sourceRect.height - margins.top - margins.bottom);
    srcSlices[4] = Rect(sourceRect.x + margins.left, sourceRect.y + margins.top,
        sourceRect.width - margins.left - margins.right,
        sourceRect.height - margins.top - margins.bottom);
    srcSlices[5] = Rect(sourceRect.x + sourceRect.width - margins.right, sourceRect.y + margins.top,
        margins.right, sourceRect.height - margins.top - margins.bottom);
    srcSlices[6] = Rect(sourceRect.x, sourceRect.y + sourceRect.height - margins.bottom,
        margins.left, margins.bottom);
    srcSlices[7] = Rect(sourceRect.x + margins.left, sourceRect.y + sourceRect.height - margins.bottom,
        sourceRect.width - margins.left - margins.right, margins.bottom);
    srcSlices[8] = Rect(sourceRect.x + sourceRect.width - margins.right,
        sourceRect.y + sourceRect.height - margins.bottom, margins.right, margins.bottom);
...
void D3D11Renderer::renderImageBatch(const std::vector<size_t>& commandIndices,
...
void D3D11Renderer::renderTextBatches(const std::vector<TextVertex>& allVertices,
...
void D3D11Renderer::renderLine(const Vec2& start, const Vec2& end,
                               const Vec4& color, float width)
{
    ...
    cbData.pSysMem = &uniforms;
    m_device->CreateBuffer(&cbDesc, &cbData, &uniformBuffer);
    
    UINT stride = sizeof(ShapeVertex);
    UINT offset = 0;
    m_context->IASetVertexBuffers(0, 1, &vertexBuffer, &stride, &offset);
    m_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    m_context->VSSetConstantBuffers(0, 1, &uniformBuffer);
...
void D3D11Renderer::renderTriangle(const Vec2& p1, const Vec2& p2, const Vec2& p3,
...
void D3D11Renderer::renderCircle(const Vec2& center, float radius,
                                 const Vec4& color, float borderWidth, bool filled)
{
