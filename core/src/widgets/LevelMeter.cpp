...
float MeterDimensions::getTotalWidth(size_t channelCount) {
    return getChannelGroupWidth(channelCount) + SCALE_WIDTH;
}
...
bool MeterThresholds::isValid() const {
    return normalToWarning < warningToPeak && warningToPeak < peakIndicator;
}
...
void MeterConfig::setThresholds(const MeterThresholds& thresholds) {
    if (thresholds.isValid()) {
        thresholds_ = thresholds;
    }
}

void MeterConfig::setBehavior(const MeterBehavior& behavior) {
    behavior_ = behavior;
}

bool MeterConfig::isValid() const {
    return thresholds_.isValid() &&
           behavior_.decayRateDbPerSec > 0.0f &&
           behavior_.peakHoldTimeMs >= 0.0f &&
           behavior_.updateEpsilon > 0.0f;
}

MeterConfig MeterConfig::createDefault() {
    return MeterConfig();
}
...
float MeterScale::mapDbToPosition(float db) const {
    return dbToPos_ ? dbToPos_(clampDb(db, minDb_, maxDb_)) : 0.0f;
}
...
MeterScale MeterScale::create(ScaleType type) {
    return MeterScale(type);
}
...
void MeterScale::initializeSamplePeak() {
    ...
    ticks_ = {
        {0.0f, mapDbToPosition(0.0f), "0", true},
        {-5.0f, mapDbToPosition(-5.0f), "5", true},
        {-10.0f, mapDbToPosition(-10.0f), "10", true},
        {-15.0f, mapDbToPosition(-15.0f), "15", true},
        {-20.0f, mapDbToPosition(-20.0f), "20", true},
        {-25.0f, mapDbToPosition(-25.0f), "25", true},
        {-30.0f, mapDbToPosition(-30.0f), "30", true},
        {-35.0f, mapDbToPosition(-35.0f), "35", true},
        {-40.0f, mapDbToPosition(-40.0f), "40", true},
        {-50.0f, mapDbToPosition(-50.0f), "50", true},
        {-60.0f, mapDbToPosition(-60.0f), "60", true}
    };
}
...
void MeterScale::initializeK12() {
    ...
    dbToPos_ = [this](float db) {
        return linearMap(db, minDb_, maxDb_, 0.0f, 1.0f);
    };
    
    posToDb_ = [this](float pos) {
        return linearMap(pos, 0.0f, 1.0f, minDb_, maxDb_);
    };
    
    ticks_ = {
        {12.0f, mapDbToPosition(12.0f), "+12", true},
        {6.0f, mapDbToPosition(6.0f), "+6", true},
        {0.0f, mapDbToPosition(0.0f), "0", true},
        {-6.0f, mapDbToPosition(-6.0f), "-6", true},
        {-12.0f, mapDbToPosition(-12.0f), "-12", true},
        {-18.0f, mapDbToPosition(-18.0f), "-18", true},
        {-24.0f, mapDbToPosition(-24.0f), "-24", true},
        {-36.0f, mapDbToPosition(-36.0f), "-36", true},
        {-48.0f, mapDbToPosition(-48.0f), "-48", true},
        {-60.0f, mapDbToPosition(-60.0f), "-60", true}
    };
}
...
void MeterScale::initializeK14() {
    ...
    ticks_ = {
        {14.0f, mapDbToPosition(14.0f), "+14", true},
        {6.0f, mapDbToPosition(6.0f), "+6", true},
        {-2.0f, mapDbToPosition(-2.0f), "-2", true},
        {-8.0f, mapDbToPosition(-8.0f), "-8", true},
        {-14.0f, mapDbToPosition(-14.0f), "-14", true},
        {-18.0f, mapDbToPosition(-18.0f), "-18", true},
        {-24.0f, mapDbToPosition(-24.0f), "-24", true},
        {-36.0f, mapDbToPosition(-36.0f), "-36", true},
...
void MeterScale::initializeVU() {
    ...
    ticks_ = {
        {3.0f, mapDbToPosition(3.0f), "+3", true},
        {2.0f, mapDbToPosition(2.0f), "+2", true},
        {1.0f, mapDbToPosition(1.0f), "+1", true},
        {0.0f, mapDbToPosition(0.0f), "0", true},
        {-1.0f, mapDbToPosition(-1.0f), "-1", true},
        {-3.0f, mapDbToPosition(-3.0f), "-3", true},
        {-5.0f, mapDbToPosition(-5.0f), "-5", true},
        {-7.0f, mapDbToPosition(-7.0f), "-7", true},
        {-10.0f, mapDbToPosition(-10.0f), "-10", true},
        {-20.0f, mapDbToPosition(-20.0f), "-20", true}
    };
}
...
void MeterScale::initializeLinearDb() {
    ...
    for (int db = static_cast<int>(maxDb_); db >= static_cast<int>(minDb_); db -= 6) {
        ticks_.emplace_back(static_cast<float>(db), mapDbToPosition(static_cast<float>(db)),
                           std::to_string(db), true);
    }
}
...
float MeterScale::clampDb(float db, float minDb, float maxDb) {
    return std::clamp(db, minDb, maxDb);
}
...
float ChannelLevelData::clampDb(float db) const {
    return std::clamp(db, MIN_DB, MAX_DB);
}
...
void LevelDataManager::updateLevels(const std::vector<float>& levels) {
    ...
    if (levels.size() != channels_.size()) {
        setChannelCount(levels.size());
    }
    size_t count = std::min(levels.size(), channels_.size());
    for (size_t i = 0; i < count; ++i) {
        channels_[i].updateLevel(levels[i], deltaTime);
    }
}
...
void LevelDataManager::setDecayRate(float dbPerSec) {
    for (auto& channel : channels_) {
        channel.setDecayRate(dbPerSec);
    }
    if (controlVoltageData_) {
        controlVoltageData_->setDecayRate(dbPerSec);
    }
}

void LevelDataManager::setPeakHoldTime(float timeMs) {
    for (auto& channel : channels_) {
        channel.setPeakHoldTime(timeMs);
    }
    if (controlVoltageData_) {
        controlVoltageData_->setPeakHoldTime(timeMs);
    }
}

void LevelDataManager::setChannelCount(size_t count) {
    if (count != channels_.size()) {
...
void BlendedColorCache::initializeTextures() {
    ...
    for (size_t i = 0; i < 12; ++i) {
        warningTexture_.pixels[i] = Vec4::FromRGBA(
            warningData[i][0], warningData[i][1], warningData[i][2], 255);
        normalTexture_.pixels[i] = Vec4::FromRGBA(
            normalData[i][0], normalData[i][1], normalData[i][2], 255);
    }
}
...
void MeterRenderer::renderChannels(RenderList& cmdList, const Vec2& startPos,
    ...
    if (showControlVoltage) {
        renderControlVoltage(cmdList, startPos, totalSize, levelData);
    }
}
...
std::vector<ChannelRenderInfo> MeterRenderer::calculateChannelLayout(
    const Vec2& startPos, const Vec2& totalSize, const LevelDataManager& levelData) const {
    
    std::vector<ChannelRenderInfo> infos;
    size_t totalChannelCount = levelData.getChannelCount();
    if (totalChannelCount == 0) return infos;
    
    float peakAndSpacing = MeterDimensions::PEAK_INDICATOR_HEIGHT +
                          MeterDimensions::PEAK_INDICATOR_SPACING;
    Vec2 meterStartPos(startPos.x + MeterDimensions::SCALE_WIDTH,
                      startPos.y + peakAndSpacing);
    Vec2 meterSize(totalSize.x - MeterDimensions::SCALE_WIDTH,
                  MeterDimensions::DEFAULT_HEIGHT);
    ...
    for (size_t i = 0; i < totalChannelCount; ++i) {
        ...
        info.peakIndicatorRect = Rect(info.meterRect.x, startPos.y,
                                      info.meterRect.width,
                                      MeterDimensions::PEAK_INDICATOR_HEIGHT);
...
void MeterRenderer::renderChannelBackground(RenderList& cmdList, const Rect& rect,
    ...
    Rect innerRect(rect.x + 1.0f, rect.y + 1.0f,
    ...
    if (normalHeight > 0.0f) {
        Rect normalRect(innerRect.x, innerRect.y + innerRect.height - normalHeight,
                       innerRect.width, normalHeight);
    ...
    if (warningHeight > 0.0f) {
        Rect warningRect(innerRect.x, innerRect.y + innerRect.height - normalHeight - warningHeight,
...
void MeterRenderer::renderChannelFill(RenderList& cmdList, const Rect& rect, float level01,
    ...
    if (level01 > 0.0f && warningThreshold01 > 0.0f) {
        ...
        if (normalFill > 0.0f) {
            ...
            Rect normalFillRect(innerRect.x, currentBottom - normalHeight,
                               innerRect.width, normalHeight);
...
void MeterRenderer::renderControlVoltage(RenderList& cmdList, const Vec2& startPos,
    ...
    Rect cvRect(cvPos.x, cvPos.y, 5.0f, MeterDimensions::DEFAULT_HEIGHT);
    
    float currentLevel = levelData.getControlVoltageLevel();
    float zeroDbPos = scale_->mapDbToPosition(0.0f);
    float minusThirtyFiveDbPos = scale_->mapDbToPosition(-35.0f);
    
    float displayHeight = std::abs(zeroDbPos - minusThirtyFiveDbPos) * MeterDimensions::DEFAULT_HEIGHT;
    float topOffset = (1.0f - zeroDbPos) * MeterDimensions::DEFAULT_HEIGHT;
    
    Rect actualRect(cvRect.x, cvRect.y + topOffset, cvRect.width, displayHeight);
    Rect innerRect(actualRect.x + 1.0f, actualRect.y + 1.0f,
                   actualRect.width - 2.0f, actualRect.height - 2.0f);
    
    if (innerRect.width > 0 && innerRect.height > 0) {
        cmdList.fillRect(innerRect, colors.bgNormal);
        
        if (currentLevel < 0.0f && currentLevel > -35.0f) {
            float fillRatio = (0.0f - currentLevel) / (0.0f - (-35.0f));
            ...
            if (fillRatio > 0.0f) {
                ...
                if (fillRect.width > 0 && fillRect.height > 0) {
                    cmdList.fillRect(fillRect, colors.levelWarning);
                }
            }
        } else if (currentLevel <= -35.0f) {
            cmdList.fillRect(innerRect, colors.levelWarning);
        }
    }
...
void MeterRenderer::drawPeakLine(RenderList& cmdList, const Rect& rect, float peakLevel01,
                                const Vec4& color, float lineHeight) {
    if (rect.width <= 0 || rect.height <= 0 || peakLevel01 <= 0.0f) return;
    ...
    Rect peakRect(rect.x + 1.0f, adjustedPeakY - halfHeight,
                 rect.width - 2.0f, lineHeight);
...
void MeterRenderer::drawInternalScaleTicks(RenderList& cmdList, const Rect& rect,
    ...
    for (const auto& tick : ticks) {
        ...
        switch (thresholds.getRegion(tick.db)) {
            case MeterThresholds::PEAK:
                scaleColor = isActive ? colors.internalScalePeakActive : colors.internalScalePeakInactive;
                break;
            case MeterThresholds::WARNING:
                scaleColor = isActive ? colors.internalScaleWarningActive : colors.internalScaleWarningInactive;
                break;
            default:
                scaleColor = isActive ? colors.internalScaleNormalActive : colors.internalScaleNormalInactive;
                break;
        }
...
void MeterRenderer::drawScaleTicks(RenderList& cmdList, const Rect& scaleRect,
    ...
    for (const auto& tick : ticks) {
        ...
        if (tickStart >= scaleRect.x) {
            cmdList.drawLine(Vec2(tickStart, tickY), Vec2(tickEnd, tickY),
                           colors.scaleColor, tickThickness);
        }
...
void LevelMeter::addDrawCommands(RenderList& commandList, const Vec2& offset) const {
    ...
    mutableRenderer.renderChannels(commandList, absPos, renderSize,
                                   levelData_, showControlVoltage_);
    ...
    if (!channelInfos.empty()) {
        mutableRenderer.renderScale(commandList, scaleRect, channelInfos[0].meterRect);
    }
...
size_t LevelMeter::getChannelCount() const {
    return levelData_.getChannelCount();
}

Vec2 LevelMeter::getRecommendedSize() const {
...
void LevelMeter::updateControlVoltage(float levelDb) {
    levelData_.updateControlVoltage(levelDb);
}

void LevelMeter::setShowControlVoltage(bool show) {
    showControlVoltage_ = show;
}

bool LevelMeter::getShowControlVoltage() const {
    return showControlVoltage_;
}
