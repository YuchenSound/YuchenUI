...
/** @file MenuItem.cpp
    ...
    - Separator items require no text or callback
    - Submenu items must have both text and valid submenu pointer
    - Radio items must have non-negative group ID
    - Checkable and radio items track checked state
    - Callback invoked synchronously when triggered
    - Type-specific validation in isValid()
*/
...
{
    // Separator requires no text or callback
    if (m_type == MenuItemType::Separator) return true;
    
    // Submenu requires text and valid submenu pointer
    ...
    // Radio items require non-negative group ID
    if (m_type == MenuItemType::Radio && m_radioGroup < 0) return false;
